...
def task_list(request):
    ...
    if search_query:
        tasks = Task.objects.filter(user = request.user, title__icontains = search_query).order_by('status', '-id')
    else:
        # Retrieves in ascending order refering to ID
        # tasks = Task.objects.filter(user = request.user)

        # To Retrieve in descending order and pending tasks at top
        tasks = Task.objects.filter(user = request.user).order_by('status', '-id')
